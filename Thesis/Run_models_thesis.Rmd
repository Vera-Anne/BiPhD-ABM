...
p<-ggplot(df_psych_sum %>% filter((id == "fat_res" | id == "stom_con" | id == "fat_loss_r"| id == "caches"), (model_type == "11" | model_type == "12" | model_type =="131" |model_type == "132"), (env == 8 | env == 6), timestep_within_day<=24), 
...
non_h_col3<-c("#283F80",  "#238242", "#CA5D05")
p <- ggplot(df %>% filter(id == "alive", 
                          (model_type == "131" | model_type =="132" | model_type =="532" | model_type =="632"), 
                          (env == 8 | env == 6)), 
            aes(y = value, x = timestep, col = var_type, lty = hoard_type)) +
  geom_line(size = 0.75) +
  facet_wrap(~food_pred, ncol = 2) +
...
# Save the plot with specified size and resolution
ggsave("ch8.png", plot = p, width = 7, height = 4.5, dpi = 500)  # Specify the size in inches and resolution
...
ch8_beh<-df%>%
  filter((id == "rest" | id == "eat" ))%>%
  filter(model_type == "11" | model_type =="51" | model_type =="61")%>%
  filter((env == 8 | env == 6))
df_beh_sum<-ch8_beh%>%
    mutate(id = case_when(
    id == "eat_hoard" ~ "Hoard", 
...
p<-ggplot(df_beh_sum %>% filter((id == "rest" | id == "eat" ), (model_type == "11" | model_type =="51" | model_type =="61"), (env == 8 | env == 6), timestep_within_day<=23), 
       aes(y = mean_value, x = timestep_within_day, col = var_type)) +
  geom_line(size = 0.75) +
  facet_grid(vars(id), vars(food_pred),  labeller = labeller(id = id_labels)) +  # Rename facet labels
  xlab("Hour in day") +  # Change x-axis label to "Days"
