...
* `num_birds`: specify the number of individual birds you want to run each model for (default = 1000)

* `daylight_h`: specify how many daylight hours you want (default = 8)

* `days`: specify how many days you want in your simulation (default = 30)


```{r set which model to run, include=TRUE, warning=FALSE,echo=F, message=FALSE}
models<-c("32")#c("11", "21", "31", "12", "22", "32")
int_var<-"temp"
...
```{r loop through models to run, include=F, warning=F, message=F, eval=T}
for (i in 1:length(models)){
  ...
  cur_out<-list(output_env_func, paste(models[i]))
  env_func_out_list[i]<-cur_out
...
```{r pull existing results, include=T, warning=F, message=F, eval=F}
...
for (i in 1:length(env_func_out_list)){
  
  if (i==1){
    agg_list<-list()
  }
  # Select hte current list with the 12 dataframes 
  cur_mod<-env_func_out_list[[i]][[2]]
  # add the env row 
  ...
  agg_cur_mod<-cur_mod%>%
    group_by(id, timestep_within_day, env_id)%>%
    summarise(mean_value_day=mean(value, na.rm=T), 
              mean_ts_within_day=mean(timestep_within_day))%>%
...
```{r check mean across environment for all values, include=T, echo=F,warning=F, message=F}
