...
load("simulation_results_Zurich_6b.Rda")
...
# general performance 
SC_1<-c(3257,	3752)#,	3123,	2653)
#FR_2<-c(2558,	3431,	2850,	2099)
#FCR_3<-c(1851,	2264,	2088,	1989)
FR_FCR_4<-c(3078,	4005)#,	3029,	2132)
SC_FR_5<-c(3258,	3694)#,	5266,	2441)
SC_FCR_6<-c(3285,	3298)#,	3654,	2073)
df_HL<-as.data.frame(rbind(SC_1, FR_FCR_4, SC_FR_5, SC_FCR_6))
df_HL$proxy<-row.names(df_HL)
colnames(df_HL)<-c("NH", "LH", "proxy")

HL_long<-tidyr::gather(df_HL, type, measurement, NH, LH,  factor_key = T)
HL_long$proxy<-ordered(HL_long$proxy, levels = c("SC_1", "FR_FCR_4", "SC_FR_5", "SC_FCR_6"))
...
phys_df<-df_out%>%
  filter(id %in% phys)
phys_df$proxy<-ordered(phys_df$proxy, levels = c("SC", "FR-FLR", "SC-FR", "SC-FLR"))
...
p<-ggplot(surv_df, aes(x = timestep, y = value, lty = as.factor(hoarding_type))) +
        ...
        #colScale
        #scale_colour_manual(values = c(lightbrown, darkbrown, lightgreen, darkblue))
...
p<-ggplot(HL_long, aes(x =as.factor(type), y = measurement,  fill = type))+
  ...
  scale_fill_manual(values = c("#D8B70A", "#02401B", "#81A88D", "#972D15"))+
...
p1<-ggplot(sum_bp, aes(x = timestep, y = mean_alive,lty = as.factor(hoarding_type)))+
        geom_line(aes(col = proxy), lwd = 1) +
...
p2<-ggplot(sum_bp_x, aes(x = timestep, y = mean_alive,lty = as.factor(hoarding_type)))+
        geom_line(aes(col = proxy), lwd = 1) +
        facet_wrap(~distribution, nrow = 2)+
        ggtitle("Survival") +
        xlab("Timestep (20 min)") +
