...
# 
# # run for 30 days 
NonHoarding_function(2160,100,-5,-5,0.2,1,3,6, 0)
# rest_or_forage(2160,100,-5,-5,0.2,1,2,4)
# rest_or_forage(2160,100,-5,-5,0.2,1,2.5,6)
NonHoarding_function(2160,100,-10,-5,0.2,1,3,6, 0)
# 
# # vary the forage sc threshold 
NonHoarding_function(2160,100,-5,-5,0.2,1,3,6, 0)
# rest_or_forage(2160,100,-5,-5,0.4,1,3,6, 0)
# rest_or_forage(2160,100,-5,-5,0.3,1,3,6, 0)
NonHoarding_function(2160,100,-5,-5,0.1,1,3,6, 0)

NonHoarding_function(2160,100,-15,-15,0.2,3,3,6, 0)
NonHoarding_function(2160,100,-15,-15,0.2,2,3,6, 0)
NonHoarding_function(2160,100,-15,-15,0.2,1,3,6, 0)
...
NonH_opt_th_sc(2160,100,-5,-5,1,3,6,1, 0, 0.4)         # full version 
NonH_opt_th_sc(360,50,-5,-5,1,3,6,1, 0, 0.4)           # 5 day version with 50 birds (quicker to run)
NonH_opt_th_sc(2160,100,-5,-5,1.5,3,6,1, 0, 0.4)
NonH_opt_th_sc(100,5,-5,-5,1.5,3,6,1, 0, 0.4)           # really fast version 
# What if there isnt as much food? 
NonH_opt_th_sc(2160,100,-5,-5,1,2,4,1, 0, 0.4)         # everybody dies :'( 
NonH_opt_th_sc(2160,100,-5,-5,1,2.5,5,1, 0, 0.4)       # in between result? 

# what if we vary the temperatures 
NonH_opt_th_sc(2160,100,-10,-10,1,3,6,1, 0, 0.4)       # colder
NonH_opt_th_sc(2160,100,10,10,1,3,6,1, 0, 0.4)         # warmer 
NonH_opt_th_sc(2160,100,-15,-15,1,3,6,1, 0, 0.4)       # very cold  
NonH_opt_th_sc(2160,100,-12,-12,1,3,6,1, 0, 0.4)        
...
NonH_opt_th_sc(2160,100,-10,-10,1,3,6,1, 0.1, 0.3) 
NonH_opt_th_sc(2160,100,-12,-12,1,3,6,1, 0.13, 0.27) 

# Change the TH-fr around 
NonH_opt_th_sc(2160,100,-5,-5,1,3,6,1, 0, 0.4)         # full version
NonH_opt_th_sc(2160,100,-5,-5,0.5,3,6,1, 0, 0.4)         # full version
...
##################################
NonH_opt_th_fr(2160,100,-5,-5,0.2,3,6,1, 0, 4)         # full version 
NonH_opt_th_fr(360,10,-5,-5,0.2,3,6,1, 0, 4)         # fast version 
NonH_opt_th_fr(2160,100,-5,-5,0.2,3,6,1, 0, 2)         # smaller range 
NonH_opt_th_fr(360,10,-5,-5,0.2,3,6,1, 0, 4)          # really fast version 
# play with temperatures a bit
NonH_opt_th_fr(2160,100,-10,-10,0.2,3,6,1, 0, 4)  
NonH_opt_th_fr(2160,100,-12,-12,0.2,3,6,1, 0, 4)  
NonH_opt_th_fr(2160,100,-15,-15,0.2,3,6,1, 0, 4) 
# again, it looks like the colder temperatures mostly prevent from foraging too much 

# how about food 
NonH_opt_th_fr(2160,100,-5,-5,0.2,2, 4,1, 0, 4) 
NonH_opt_th_fr(2160,100,-5,-5,0.2,2.5, 5,1, 0, 4) 

# play with the sc 
NonH_opt_th_fr(2160,100,-5,-5,0.09,3,6,1, 0, 4)         # full version 
...
NonH_opt_th_sc_and_fr<-function(T, N, temp_day, temp_night, num_food_mean, num_food_max, noplot, th_sc_min, th_sc_max, th_fr_min, th_fr_max){
  ...
  for (th_sc in 1:length(th_forage_sc)){
    ...
    for (th_fr in 1:length(th_forage_fr)){
      ...
      NonHoarding_function(T, N, temp_day, temp_night, current_th_sc, current_th_fr, num_food_mean, num_food_max, noplot)
      # add to the previously created matrix 
...
NonH_opt_th_sc_and_fr(2160,100,-5,-5,2,4,1, 0, 0.4, 0, 4)         # full version 
...
# Remove the optimisation type if you want to just run hte simulation 
rm(opt_type)
Hoarding_function<-function(TS, N, temp_day, temp_night, th_forage_sc, th_forage_fr, num_food_mean, num_food_max, noplot){
  ...
  if (exists('opt_type')){                                                      # Check if you are optimising or just running 
    if(opt_type=='th_sc'){                                                      # If so, are you optimising th_sc? 
      print(paste0('H function did run for sc:', current_th_sc ))
    }
    if(opt_type=='th_fr'){                                                      # Or are you optimising th_fr? 
      print(paste0('H function did run for fr:', current_th_fr ))
    }
...
Hoarding_function(2160, 100, -5, -5, 0.2, 1, 3, 6, 0)         # general version (default values)
Hoarding_function(4320, 100, -5, -5, 0.2, 1, 3, 6, 0)         # 60 days instead 
Hoarding_function(2160, 100, -5, -10, 0.2, 1, 3, 6, 0)        # colder
Hoarding_function(2160, 100, -10, -5, 0.2, 1, 3, 6, 0)        # colder
Hoarding_function(2160, 100, -10, -10, 0.2, 1, 3, 6, 0)       # colder
Hoarding_function(102, 10, -5, -5, 0.2, 1, 3, 6, 0)           # shorter 
...
# now run the optimization function 
Hoard_opt_th_sc(2160,100,-5,-5,1,3,6,1, 0, 0.4)         # full version 
Hoard_opt_th_sc(360,50,-5,-5,1,3,6,1, 0, 0.4)           # 5 day version with 50 birds (quicker to run)
Hoard_opt_th_sc(100,5,-5,-5,1,3,6,1, 0, 0.4)           # very short 
# What if there isnt as much food? 
Hoard_opt_th_sc(2160,100,-5,-5,1,2,4,1, 0, 0.4)         # mean of 2, maximum of 4
Hoard_opt_th_sc(2160,100,-5,-5,1,2.5,5,1, 0, 0.4)       # mean of 2.5, maximum of 5  

# what if we vary the temperatures 
Hoard_opt_th_sc(2160,100,-10,-10,1,3,6,1, 0, 0.4)       # colder
Hoard_opt_th_sc(2160,100,10,10,1,3,6,1, 0, 0.4)         # warmer 
Hoard_opt_th_sc(2160,100,-15,-15,1,3,6,1, 0, 0.4)       # very cold  
Hoard_opt_th_sc(2160,100,-12,-12,1,3,6,1, 0, 0.4)        
# notice that when temperatures get colder, the higher thresholds (just always forage)
# dont work as well anymore 
...
###########################################
Hoard_opt_th_fr(2160,100,-5,-5,0.2,3,6,1, 0, 4)          # full version 
Hoard_opt_th_fr(100,5,-5,-5,0.2,3,6,1, 0, 4)            # fast version 

# play with sc-th
Hoard_opt_th_fr(2160,100,-5,-5,0.09,3,6,1, 0, 4)          # full version 

# play with temperatures a bit
Hoard_opt_th_fr(2160,100,-10,-10,0.2,3,6,1, 0, 4)  
Hoard_opt_th_fr(2160,100,-12,-12,0.2,3,6,1, 0, 4)  
Hoard_opt_th_fr(2160,100,-15,-15,0.2,3,6,1, 0, 4) 
# again, it looks like the colder temperatures mostly prevent from foraging too much 

# how about food 
Hoard_opt_th_fr(2160,100,-5,-5,0.2,2, 4,1, 0, 4) 
Hoard_opt_th_fr(2160,100,-5,-5,0.2,2.5, 5,1, 0, 4) 
...
Hoard_opt_th_sc_and_fr(100,5,-10,-10,3,6,1, 0, 0.4, 0, 4)         # superfast version
# what if it is a bit colder
Hoard_opt_th_sc_and_fr(2160,100,-10,-10,3,6,1, 0, 0.4, 0, 4)         # colder
Hoard_opt_th_sc_and_fr(2160,100,-15,-15,3,6,1, 0, 0.4, 0, 4)         # more colder 

# what if we have fewer food? 
Hoard_opt_th_sc_and_fr(2160,100,-5,-5,2,4,1, 0, 0.4, 0, 4)         # mean 2 max 4 food items 
