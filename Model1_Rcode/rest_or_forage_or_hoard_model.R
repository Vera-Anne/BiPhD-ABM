...
# need to see if the cut off values for the retrieval threshold make sense now in new model 
...
rest_or_eat_or_eatHoard<-function(T, N, temp_day, temp_night, th_forage_sc, th_forage_fr, num_food_mean, num_food_max, noplot){
  ...
  # Set up the environment: run environment function 
  set_up_env(T, N, temp_cur, num_food_mean, num_food_max, num_cache_min, num_cache_max)              # could be a problem that temp-cur is not known atm. 
        ...
        if(dayOrNight==0){
          ...
          if (((mat_sc[i,t]) < th_forage_sc) && (mat_fr[i,t]<th_forage_fr)){
            ...
            #       3. Find new item and eat it till full, hoard after this 
              ...
              if (mat_fr[i,t]<cur_th_retrieval){
                # The bird will retrieve 
                ...
                food_cur<<-cur_caches_retrieved*food_item
                mat_caches[i,(t)]<<-(mat_caches[i,t])+(food_cur)
                ...
                THIS IS WHERE THE OTHER FORAGING CODE STARTS 
