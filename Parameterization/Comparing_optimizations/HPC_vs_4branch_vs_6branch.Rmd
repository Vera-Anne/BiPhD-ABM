...
plot_x1<-ggplot(gold_list2[[i]][[1]], aes(x=th, y=mean_HL))+
  ...
  geom_point(data=gold_list2[[i]][[2]], aes(x=th, y=mean_HL), col="goldenrod", size=5)+
  # Best value 4branch 
...
# Make the graph 
    th1<-unique(gold_list1[[i]]$th1)
    th2<-unique(gold_list1[[i]]$th2)
    grid<-expand.grid(th1, th2)
    colnames(grid)<-c("th1", "th2")
    grid<-left_join(grid, gold_list1[[1]])
    grid[is.na(grid)]<-0
    
  # create a matrix with the values for HL
      HL_matrix<-matrix(data=grid$mean, ncol=49)
      # Plot
      p<-plot_ly(x=th1, y=th2, z=HL_matrix, type="surface")
