...
```{r x.3 subset 1 }
models_x3<-c("131", "231", "331", "132", "232", "332")
...
# get the results from golden search phase 1 
    # Phase 1 contains 1 single run for each of the combinations 
      setwd(paste0(gold_folder, "/", model, "/phase_1/"))
     # get the files in the folder 
      filenames <- list.files(pattern="*.Rda", full.names=TRUE)
     # load the file 
        load(filenames[length(filenames)])
...
# Get the results from the 6-branch 
  ...
  # Preparation for the graph 
    ...
    # Then for everyone: 
          ...
          fig <- plot_ly(gold_list1,x = ~th1,y = ~th2,z = ~th3,color = ~mean, marker = list(symbol = 'circle', 
                                       sizemode = 'diameter'),sizes = c(0.01, 150))
          
          
          
          
          fig <- fig %>% add_markers()
          fig <- fig %>% layout(scene = list(title = list(title = paste('model', model)),
                               yaxis = list(title = 'th2'),
                               zaxis = list(title = 'th3'), 
                               xaxis = list(title = "th1")))
...
fig
          ...
          # merge so we have complet ematrix 
          merged<-merge(df_merge, gold_list1, all=T)
          merged[is.na(merged)]<-0
...
#th1<-linspace(-0.6, 0.6, 50)
#th2<-linspace(-0.6, 0.6, 50)
#th3<-linspace(-0.6, 0.6, 50)
th1<-linspace(0, 4, 50)
th2<-linspace(0, 4, 50)
th3<-linspace(0, 4, 50)
# th1<-linspace(0, 0.4, 50)
# th2<-linspace(0, 0.4, 50)
# th3<-linspace(0, 0.4, 50)
df_merge<-expand.grid(th1,th2, th3)
colnames(df_merge)<-c("th1", "th2", "th3")

# merge so we have complet ematrix 
merged<-merge(df_merge, HL_df, all=T)
merged[is.na(merged)]<-0
fig <- plot_ly(HL_df, x = ~HL_df$th1, y = ~HL_df$th2, z = ~HL_df$th3, color = ~mean,size = ~mean, 
             marker = list(symbol = 'circle', sizemode = 'diameter'), sizes = c(0.1, 15))
fig <- fig %>% add_markers()
fig <- fig %>% layout(scene = list(title = list(title = paste('model', model_type)),
                     yaxis = list(title = 'th2'),
                     zaxis = list(title = 'th3'), 
                     xaxis = list(title = "th1")))
...
# Add specific points (gold, branch)
th1_dummy<-0.5714286
th2_dummy<-1.22449
th3_dummy<-1.306122
mean_dummy<-2293.676
points_df<-as.data.frame(cbind(th1_dummy, th2_dummy, th3_dummy, mean_dummy))
fig <- fig %>% add_markers(data = points_df, x = ~th1_dummy, y = ~th2_dummy, z = ~th3_dummy, color = I('#ffcc00'))
fig
th1_dummy<-0.46875
th2_dummy<-2.15625
th3_dummy<-3.03125
mean_dummy<-2099.277
points_df<-as.data.frame(cbind(th1_dummy, th2_dummy, th3_dummy, mean_dummy))
fig<-fig %>% add_markers(data = points_df, x = ~th1_dummy, y = ~th2_dummy, z = ~th3_dummy, color = I("chocolate"))
fig
