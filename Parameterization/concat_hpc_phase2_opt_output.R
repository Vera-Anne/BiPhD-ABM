...
outcome_concat<- foreach(j=1:length(filenames), .combine='c') %dopar% {
    ...
    halflife_list[[1]][3]<-env_results[[3]][4]      # th num 
    halflife_list[[1]][4]<-env_results[[3]][7]      # rep number 
    halflife_list[[1]][5]<-env_results[[3]][8]      # total number of options possible for a threshold (default = 50)
    halflife_list[[1]][6]<-env_results[[3]][9]      # threshold 1 
    halflife_list[[1]][7]<-env_results[[3]][10]      # threshold 2 
    
    # For x.3.y models this should be added: 
    #halflife_list[[1]][5]<-env_results[[3]][11]      # threshold 3 
