...
# Input variables 
  ...
  # current threshold 
  cur_th<-as.numeric(args[7])
  
  # model type 
  modelType<-as.numeric(args[8])
  
  # Output directory 
  out_dir<-args[9]
  
  # rep number
  rep_num<-as.numeric(args[10])
...
##########################
      ...
      outut<-cbind(output_env_func[[1]][1], cur_th)
...
# make sure to attach the threshold to the dataframe 
  save(out, file=paste0('out_4branch_run', modelType, '_level', level, "_", format(Sys.time(), "%Y-%m-%d_%H_%M_%S"),'.Rda'))
