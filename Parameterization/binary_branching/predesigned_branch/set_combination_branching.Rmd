...
# Branching for 6 
    branch_func_6<-function(range_min, range_max){
      # determine stepsize
      step_size<<-((range_max-range_min)/6)
...
out_sub1_x2<-tree_func_x2(model_type = 22, branch_num = 4)
```

The subset 1 x.3 models

```{r run branching subset 1 x.2, include=TRUE, message=F, warning=F}
out_sub1_x3<-tree_func_x3(model_type = 232, branch_num = 4)
```
...
```{r run subset 2 x1}
out_sub2_x1<-tree_func_sub2_x1(model_type=51, branch_num = 4)
```
