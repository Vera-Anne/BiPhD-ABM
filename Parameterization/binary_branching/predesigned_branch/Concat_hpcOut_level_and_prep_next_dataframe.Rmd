...
```{r settings}
model_type<-42
level<-0       # The level that you are concatenating (the level that just ran on the HPC) - put 0 if you want to generate for level 1 
subset<-2
```
...
# Extract best values from previous level 
  ...
  # For the best th from level 2
   ...
   # Best of level 1
      # Set the wd to where the concatenation results are 
      setwd(paste0(general_folder, "/output_subset", subset, "/level_", (level-2), "/concat_results"))
      # Load the relevant dataframe 
      filenames <- list.files(pattern=paste0("*MOD_", model_type, "_lev", 1, ".Rda"), full.names=TRUE)
      load(filenames[length(filenames)])
      # Extract first row 
...
if(next_level_chr=="level1"){
  ...
  setwd(paste0(general_folder, "/final_results"))
      write.table(best_comb_df, file=paste0('optimal_th_sub', subset, "_x",  (str_sub(model_type, 2,2)), "_mod", 
