######################################################################################
# 23/12/2023
# ONly runs the simulation part of the branching for x.1 models 
# Vera Vinken 
...
# rep number
rep_num<-as.numeric(args[7])

# level 
level<-as.numeric(args[8])
...
output<-cbind(output_env_func[[1]], cur_th)
...
# make sure to attach the threshold to the dataframe 
  save(output, file=paste0('out_4branch', modelType, '_level', level, "_th", cur_th, "_rep", rep_num,"_", format(Sys.time(), "%Y-%m-%d_%H_%M_%S"),'.Rda'))
