...
setwd(file_folder)
save(out_x3, file=paste0("out_x3_min", start_min_th1, "_max", start_max_th1, ".Rda"))
```
...
# Non hoarder 2 variables 
    tree_func_sub2_x1<-function(start_min_v1_th1, start_max_v1_th1, start_min_v2_th1, start_max_v2_th1, branch_num, model_type){
      depth<-3
      # Start the loop that will go thrugh the number of steps as set in 'depth' 
      for (i in 1:depth){
        # Generate a list for the output 
...
out_sub2_x1<-tree_func_sub2_x1(start_min_v1_th1 = 0, start_max_v1_th1 = 0.4, start_min_v2_th1 = 0, start_max_v2_th1 = 4, model_type=51, branch_num = 4)

setwd(file_folder)
save(out_sub2_x1, file=paste0("branch_sub2_x1.Rda"))
...
out_sub2_x2<-tree_func_sub2_x2(model_type=42, branch_num = 4)
setwd(file_folder)
save(out_sub2_x2, file=paste0("branch_sub2_x2.Rda"))
      ...
      for (i in 1:depth){
        # Generate a list for the output 
          if (i==1){
            ...
            # Subset this for cases 
                  grid<-grid%>%
                    filter(case_when( model_type=="43"~v1_th1>0 & v1_th2>0 & v1_th3>0, 
                                      model_type=="53"~v1_th1>0 & v2_th1>0 & v1_th2>0 & v2_th2>0 & v1_th3>0 & v2_th3>0,
                                      model_type=="63"~v1_th1>0 & v1_th2>0 & v1_th3>0))%>%
                    filter(v1_th1<v1_th2)%>%
                    filter(v2_th1<v2_th2)%>%
...
out_sub2_x3<-tree_func_sub2_x3(model_type=43, branch_num = 4)
setwd(file_folder)
```

## Flexible code to save output 
I need flexible code that lets me save either bit of the branching that I've done 
```{r saving}
setwd()
write.table(out_sub2_x3, file=paste('best_1000_', opt_type, '_',format(Sys.time(), "%Y-%m-%d_%H_%M_%S"), '.csv'), row.names = F, col.names = F)
