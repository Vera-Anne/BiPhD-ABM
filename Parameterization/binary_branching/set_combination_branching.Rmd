...
library(strinr) # to select the 2nd digit of a number with the str_sub() function 
```
...
subset<-2
model_type<-531
level<-"level1" # set teh level that you want to extract from 
# any  previuos results 
best_comb_l1<-as.numeric(c(NA))
best_comb_l2<-as.numeric(c(NA))
...
if(level=="level1"){
  ...
  selected_df<-tree_output_list[[level]][[paste(best_comb_l1)]]
    # You're there 
   write.table(selected_df, file=paste0('th_HPC_sub', subset, "_x",  (str_sub(model_type, 2,2)), "_mod", model_type, '_', level, '.csv'), row.names = F, col.names = F)
}else if(level=="level3"){
    selected_df<-tree_output_list[[level]][[paste(best_comb_l1)]][[paste(best_comb_l2)]]
    # You're there 
   write.table(selected_df, file=paste0('th_HPC_sub', subset, "_x",  (str_sub(model_type, 2,2)), "_mod", model_type, '_', level, '.csv'), row.names = F, col.names = F)
}
