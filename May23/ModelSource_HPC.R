...
# model 1.1 HPC 
mod_4_1_hpc<-function(days, N, env_type, th_forage_fr, th_forage_flr, daylight_h){
  # Make the variables global 
  days<<-days
  N<<-N
  env_type<<-env_type
...
env_func_4_1_par_hpc<-function(days, N, th_forage_fr, th_forage_flr, daylight_h, modelType){
  # For parallel computing 
  require(doParallel)
  require(foreach)
  numCores<-(12)
  ...
  outcome_env_4_1_par<- foreach(i=1:num_env, .packages = c( "truncnorm", "purrr")) %dopar% {
    # Single model 
    mod_4_1_hpc(days = days, N = N, env_type = i, th_forage_fr = th_forage_fr, th_forage_flr = th_forage_flr, daylight_h = daylight_h)
  }
...
} # end environment function loop - 4.1 HPC 
...
# model 4.2 - HPC 
mod_4_2_hpc<-function(days, N, env_type, th_forage_fr1, th_forage_fr2, th_forage_flr1, th_forage_flr2, daylight_h){
  # make some of the variables global for saving 
  days<<-days
  N<<-N
  env_type<<-env_type
...
# environment loop paralelel - HPC 
env_func_4_2_par_hpc<-function(days, N, th_forage_fr1, th_forage_fr2, th_forage_flr1, th_forage_flr2, daylight_h, modelType){
  # For parallel computing
  require(doParallel)
  require(foreach)
  numCores<-(12)
  ...
  outcome_env_4_2_par<- foreach(i=1:num_env, .packages = c( "truncnorm", "purrr")) %dopar% {
    ...
    # single model 
    mod_4_2_hpc(days = days, N = N, env_type = i, th_forage_fr1 = th_forage_fr1, th_forage_fr2 = th_forage_fr2,th_forage_flr1 = th_forage_flr1, th_forage_flr2 = th_forage_flr2, daylight_h = daylight_h)
  }
...
} # end environment function loop for model 4.2 -HPC

#########################################################################
##   Model 4.3: Direct-hoarding bird, Access to Fat-reserve and FLR  ##
#########################################################################

# model 4.3.1 : direct hoarding bird with foraging up top  - HPC 
mod_4_3_1_hpc<-function(days, N, env_type, th_forage_fr1, th_forage_fr2, th_forage_fr3, th_forage_flr1, th_forage_flr2, th_forage_flr3,daylight_h){
  # make some of the variables global for saving 
  days<<-days
  N<<-N
  env_type<<-env_type
...
} # end of model 4.3.1 function - HPC


# model 4.3.2 : direct hoarding bird with foraging up top  - HPC
mod_4_3_2_hpc<-function(days, N, env_type, th_forage_fr1, th_forage_fr2, th_forage_fr3, th_forage_flr1, th_forage_flr2, th_forage_flr3,daylight_h){
  # make some of the variables global for saving 
  days<<-days
  N<<-N
  env_type<<-env_type
...
} # end of model 4.3.2 function - HPC
...
# the one that runs parallel - HPC
env_func_4_3_1_par_hpc<-function(days, N, th_forage_fr1, th_forage_fr2, th_forage_fr3,th_forage_flr1, th_forage_flr2, th_forage_flr3, daylight_h, modelType){
  # parallel computing 
  require(doParallel)
  require(foreach)
  numCores<-(12)
  ...
  outcome_env_4_3_1_par<- foreach(i=1:num_env, .packages = c( "truncnorm", "purrr")) %dopar% {
    ...
    # single model run 
    mod_4_3_1_hpc(days = days, N = N, env_type = i,th_forage_fr1 = th_forage_fr1, th_forage_fr2 = th_forage_fr2, th_forage_fr3= th_forage_fr3, th_forage_flr1 = th_forage_flr1, th_forage_flr2 = th_forage_flr2, th_forage_flr3= th_forage_flr3, daylight_h = daylight_h)
...
} # end environment function loop for 4.3.1 - HPC


# The one that runs parallel - HPC
env_func_4_3_2_par_hpc<-function(days, N, th_forage_fr1, th_forage_fr2, th_forage_fr3,th_forage_flr1, th_forage_flr2, th_forage_flr3, daylight_h, modelType){
  # for parallel computing 
  require(doParallel)
  require(foreach)
  numCores<-(12)
  ...
  outcome_env_4_3_2_par<- foreach(i=1:num_env, .packages = c( "truncnorm", "purrr")) %dopar% {
    ...
    # single model run 
    mod_4_3_2_hpc(days = days, N = N, env_type = i, th_forage_fr1 = th_forage_fr1, th_forage_fr2 = th_forage_fr2, th_forage_fr3= th_forage_fr3,th_forage_flr1 = th_forage_flr1, th_forage_flr2 = th_forage_flr2, th_forage_flr3= th_forage_flr3, daylight_h = daylight_h)
  }
...
} # end environment function loop for 4.3.2 - hpc
...
# model 5.1 HPC 
mod_5_1_hpc<-function(days, N, env_type, th_forage_sc, th_forage_fr, daylight_h){
  # Make the variables global 
  days<<-days
  N<<-N
  env_type<<-env_type
...
env_func_5_1_par_hpc<-function(days, N, th_forage_sc, th_forage_fr, daylight_h, modelType){
  ...
  outcome_env_5_1_par<- foreach(i=1:num_env, .packages = c( "truncnorm", "purrr")) %dopar% {
    # Single model 
    mod_5_1_hpc(days = days, N = N, env_type = i, th_forage_sc = th_forage_sc, th_forage_fr = th_forage_fr, daylight_h = daylight_h)
  }
...
} # end environment function loop - 5.1 HPC 
...
# model 5.2 - HPC 
mod_5_2_hpc<-function(days, N, env_type, th_forage_sc1, th_forage_sc2, th_forage_fr1, th_forage_fr2, daylight_h){
  # make some of the variables global for saving 
  days<<-days
  N<<-N
  env_type<<-env_type
...
# environment loop paralelel - HPC 
env_func_5_2_par_hpc<-function(days, N, th_forage_sc1, th_forage_sc2, th_forage_fr1, th_forage_fr2, daylight_h, modelType){
  # For parallel computing
  require(doParallel)
  require(foreach)
  numCores<-(12)
  ...
  outcome_env_5_2_par<- foreach(i=1:num_env, .packages = c( "truncnorm", "purrr")) %dopar% {
    ...
    # single model 
    mod_5_2_hpc(days = days, N = N, env_type = i, th_forage_sc1 = th_forage_sc1, th_forage_sc2 = th_forage_sc2,
...
} # end environment function loop for model 5.2 -HPC
...
mod_5_3_1_hpc<-function(days, N, env_type, th_forage_sc1, th_forage_sc2, th_forage_sc3, th_forage_fr1, th_forage_fr2, th_forage_fr3,daylight_h){
  # make some of the variables global for saving 
  days<<-days
  N<<-N
  env_type<<-env_type
...
} # end of model 5.3.1 function - HPC


# model 5.3.2 : direct hoarding bird with foraging up top  - HPC
mod_5_3_2_hpc<-function(days, N, env_type, th_forage_sc1, th_forage_sc2, th_forage_sc3, th_forage_fr1, th_forage_fr2, th_forage_fr3, daylight_h){
  # make some of the variables global for saving 
  days<<-days
  N<<-N
  env_type<<-env_type
...
} # end of model 5.3.2 function - HPC
...
# the one that runs parallel - HPC
env_func_5_3_1_par_hpc<-function(days, N, th_forage_sc1, th_forage_sc2, th_forage_sc3,th_forage_fr1, th_forage_fr2, th_forage_fr3, daylight_h, modelType){
  # parallel computing 
  require(doParallel)
  require(foreach)
  numCores<-(12)
  ...
  outcome_env_5_3_1_par<- foreach(i=1:num_env, .packages = c( "truncnorm", "purrr")) %dopar% {
    ...
    # single model run 
    mod_5_3_1_hpc(days = days, N = N, env_type = i,th_forage_sc1 = th_forage_sc1, th_forage_sc2 = th_forage_sc2, th_forage_sc3= th_forage_sc3, 
...
} # end environment function loop for 5.3.1 - HPC
...
} # end environment function loop for 5.3.2 - hpc
...
# model 6.1 HPC 
mod_6_1_hpc<-function(days, N, env_type, th_forage_sc, th_forage_flr, daylight_h){
  # Make the variables global 
  days<<-days
  N<<-N
  env_type<<-env_type
...
# environment loop paralelel HPC 
env_func_6_1_par_hpc<-function(days, N, th_forage_sc, th_forage_flr, daylight_h, modelType){
  # For parallel computing 
  require(doParallel)
  require(foreach)
  numCores<-(12)
  ...
  outcome_env_6_1_par<- foreach(i=1:num_env, .packages = c( "truncnorm", "purrr")) %dopar% {
    # Single model 
    mod_6_1_hpc(days = days, N = N, env_type = i, th_forage_sc = th_forage_sc, th_forage_flr = th_forage_flr, daylight_h = daylight_h)
  }
...
} # end environment function loop - 6.1 HPC 
...
# environment loop paralelel - HPC 
env_func_6_2_par_hpc<-function(days, N, th_forage_sc1, th_forage_sc2, th_forage_flr1, th_forage_flr2, daylight_h, modelType){
  # For parallel computing
  require(doParallel)
  require(foreach)
  numCores<-(12)
  ...
  outcome_env_6_2_par<- foreach(i=1:num_env, .packages = c( "truncnorm", "purrr")) %dopar% {
    ...
    # single model 
    mod_6_2_hpc(days = days, N = N, env_type = i, th_forage_sc1 = th_forage_sc1, th_forage_sc2 = th_forage_sc2,
...
} # end environment function loop for model 6.2 -HPC
...
mod_6_3_1_hpc<-function(days, N, env_type, th_forage_sc1, th_forage_sc2, th_forage_sc3, th_forage_flr1, th_forage_flr2, th_forage_flr3,daylight_h){
  # make some of the variables global for saving 
  days<<-days
  N<<-N
  env_type<<-env_type
...
} # end of model 6.3.2 function - HPC
...
# the one that runs parallel - HPC
env_func_6_3_1_par_hpc<-function(days, N, th_forage_sc1, th_forage_sc2, th_forage_sc3,th_forage_flr1, th_forage_flr2, th_forage_flr3, daylight_h, modelType){
  # parallel computing 
  require(doParallel)
  require(foreach)
  numCores<-(12)
  ...
  outcome_env_6_3_1_par<- foreach(i=1:num_env, .packages = c( "truncnorm", "purrr")) %dopar% {
    ...
    # single model run 
    mod_6_3_1_hpc(days = days, N = N, env_type = i,th_forage_sc1 = th_forage_sc1, th_forage_sc2 = th_forage_sc2, th_forage_sc3= th_forage_sc3, 
...
} # end environment function loop for 6.3.2 - hpc
